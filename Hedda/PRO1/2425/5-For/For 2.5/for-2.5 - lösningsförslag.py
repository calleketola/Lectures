...
antal = int(input("Hur många tal vill du multiplicera? "))
...
print(f"Summan av dina tal är: {summa}")
...
invånare = 26000
år = int(input("Vilket år är du intresserad av? "))

for i in range(år-2022):
    invånare *= 1.007*0.994
    invånare += 300-325

print(f"År {år} är det {invånare} invånare i kommunen")
...
tal = random.randit(start, stop)
Notera att värdet stop kommer med
"""
print("Övning 7+8")
gånger = random.randint(10)
for j in range(gånger)
    antal = random.randint(1,25)
    for i in range(antal):
        print(i, end=" ")
    print()
...
for i in range(0,100,2):
    jämna_tal.append(i)
for i in range(len(jämna_tal)):
    print(jämna_tal[i],end=" ")
...
for i in range(0,len(jämna_tal),3):
    jämna_tal[i] *= -1
print(f"Övning 12: {sum(jämna_tal)}")
# Alternativt
summa = 0
for i in range(len(jämna_tal)):
    summa += jämna_tal[i]
...
tal = []
for i in range(5):
    nytt = float(input(f"Tal {i+1}: "))
    tal.append(nytt)
print(max(tal), min(tal))

störst = tal[0] # Anta att det första talet är störst
minst = tal[0] # Anta att det minsta talet är minst
...
for rad in range(len(listor)): # Nu väljer jag att använda rad istället för i
    for kol in range(len(listor[rad])): # Detta gör det lättare att hålla koll på vad man gör
        print(listor[rad][kol])
